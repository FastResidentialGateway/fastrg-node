...
# ---------------------------------------------------------------------------
# Phase 28 — CHAP authentication and PAP baseline restore (Steps 115-117)
# ---------------------------------------------------------------------------
...
phase28_chap_auth() {
    ...
    bold "═══════════════════════════════════════════════════════"
    bold " Phase 28 — CHAP Authentication (Steps 115-117)"
    bold "═══════════════════════════════════════════════════════"
    ...
    if [[ $_step113_ok -eq 1 ]]; then
        pass "Step 115: CHAP dial reaches Data phase" \
            "users ${SUB_IDS[*]} reached Data phase with per-user challenge and auth-success logs"
    else
        fail "Step 115: CHAP dial reaches Data phase" "$_issue113"
    fi
    ...
    if [[ $_step114_ok -eq 1 ]]; then
        pass "Step 116: CHAP session data plane" \
            "${WAN_IP} reachable with 4 packets and 0% packet loss"
    else
        fail "Step 116: CHAP session data plane" "$_issue114"
    fi
    ...
    if [[ $_step115_ok -eq 1 ]]; then
        pass "Step 117: restore default PAP baseline" \
            "default BRAS parameters restored; users ${SUB_IDS[*]} returned to Data phase"
    else
        fail "Step 117: restore default PAP baseline" "$_issue115"
    fi
