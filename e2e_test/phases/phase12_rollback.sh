...
# ---------------------------------------------------------------------------
# Phase 12 — Config-Apply Failure → Controller Rollback (Steps 46-47)
#
...
#
#   Step 46  existing user: bad config rolled back to last-successful version
#   Step 47  new user (no DB record): bad key removed from etcd on failure
# ---------------------------------------------------------------------------
...
# USER_ID's canonical hsi fixture (Step 46) and a stray never-configured-user
# key (Step 47) to trigger controller rollback. Both are normally cleaned up
...
phase12_rollback() {
    bold "═══════════════════════════════════════════════════════"
    bold " Phase 12 — Config-Apply Failure + Rollback (Steps 46-47)"
    bold "═══════════════════════════════════════════════════════"

    # ------------------------------------------------------------------
    # Step 46 — existing user: bad config → apply fail → controller
    #           reads last-successful from DB → CAS restores etcd
    # ------------------------------------------------------------------
    info "Step 46: trigger apply failure for USER_ID=${USER_ID} and expect rollback..."
    ...
    _p13_bad=$(etcdctl_get_value "configs/${NODE_UUID}/hsi/${USER_ID}" 2>/dev/null | \
        ...
        2>/dev/null || true)
    if [[ -z "$_p13_bad" ]]; then
        fail "Step 46: config-apply rollback" "could not construct bad config from etcd"
        skip "Step 47: new-user rollback (delete)" "prerequisite failed"
        return
    fi
    ...
    if _p43_acct_by=$(_p13_wait_etcd_vlan "${USER_ID}" "${_p13_cur_vlan}" 30 2>/dev/null); then
        ...
        if [[ "$_rb_acct" == "$_p13_cur_acct" ]]; then
            pass "Step 46: config-apply rollback" \
                "etcd restored to vlan=${_p13_cur_vlan} acct=${_rb_acct} by=${_rb_by}"
        else
            fail "Step 46: config-apply rollback" \
    ...
    # ------------------------------------------------------------------
    # Step 47 — new user with no DB record: bad key should be removed
    #           (controller cannot restore a prior version → deletes it)
    # ------------------------------------------------------------------
    info "Step 47: write bad config for a brand-new user (no DB record) — expect key deleted..."
    ...
    if [[ $_p44_ok -eq 1 ]]; then
        pass "Step 47: new-user rollback (delete)" \
        ...
        _remaining=$(etcdctl_get_value "configs/${NODE_UUID}/hsi/${_new_uid}" 2>/dev/null | \
            jq -r '.config.account_name // empty' 2>/dev/null || true)
        fail "Step 47: new-user rollback (delete)" \
    ...
    # Unconditional cleanup: Step 46 wrote vlan=0 into configs/.../hsi/USER_ID,
    # and Step 47 may have left a stray key if its own fail-branch delete above
