...
# ---------------------------------------------------------------------------
# Phase 18 — DNS cache behaviour and upstream failover (Steps 71-75)
#
...
phase18_dns_cache() {
    ...
    bold "═══════════════════════════════════════════════════════"
    bold " Phase 18 — DNS Cache Behaviour + Failover (Steps 71-75)"
    bold "═══════════════════════════════════════════════════════"

    _gw=$(etcdctl_get_value "configs/${NODE_UUID}/hsi/${USER_ID}" 2>/dev/null | \
        jq -r '.config.dhcp_gateway // empty' 2>/dev/null || true)
    if [[ -z "$_gw" ]]; then
        fail "Step 71: DNS cache fill + GetDnsCache" "subscriber gateway unavailable"
        fail "Step 72: DNS cache hit with upstream stopped" "phase setup failed"
        fail "Step 73: DNS TTL expiry refetches upstream" "phase setup failed"
        fail "Step 74: DNS upstream failover" "phase setup failed"
        fail "Step 75: DNS phase cleanup + static recovery" "phase setup failed"
        return
    fi
    ...
    if ! scp $SSH_OPTS "${GRPC_CLIENT_DIR}/dns_responder.py" \
       ...
       ! _p18_start_responder 120; then
        fail "Step 71: DNS cache fill + GetDnsCache" "could not start WAN DNS responder"
        fail "Step 72: DNS cache hit with upstream stopped" "phase setup failed"
        fail "Step 73: DNS TTL expiry refetches upstream" "phase setup failed"
        fail "Step 74: DNS upstream failover" "phase setup failed"
        fail "Step 75: DNS phase cleanup + static recovery" "phase setup failed"
    ...
    if ! _p18_restart_bras "${WAN_IP}" "${_P18_SECONDARY_DNS}" || ! _p18_redial; then
        fail "Step 71: DNS cache fill + GetDnsCache" "could not negotiate WAN responder as IPCP primary DNS"
        fail "Step 72: DNS cache hit with upstream stopped" "phase setup failed"
        fail "Step 73: DNS TTL expiry refetches upstream" "phase setup failed"
        fail "Step 74: DNS upstream failover" "phase setup failed"
        fail "Step 75: DNS phase cleanup + static recovery" "phase setup failed"
    ...
    if [[ "$_dig" == "${_P18_ANSWER}" ]] && [[ "$_entry" == "${_P18_DOMAIN}" ]] && \
       [[ "$(printf '%s' "$_flush" | jq -r '.status // empty' 2>/dev/null || true)" == "ok" ]]; then
        pass "Step 71: DNS cache fill + GetDnsCache" "answer=${_P18_ANSWER}; cache entry present"
    else
        fail "Step 71: DNS cache fill + GetDnsCache" \
    ...
    if [[ "$_dig" == "${_P18_ANSWER}" ]] && [[ "$_count_after" == "$_count_before" ]]; then
        pass "Step 72: DNS cache hit with upstream stopped" \
            "cached answer returned; upstream count stayed ${_count_before}"
    else
        fail "Step 72: DNS cache hit with upstream stopped" \
            "dig='${_dig:-empty}' upstream_count=${_count_before:-?}->${_count_after:-?}"
    fi

    if ! _p18_start_responder 5; then
        fail "Step 73: DNS TTL expiry refetches upstream" "could not restart WAN DNS responder"
    else
        fastrg_grpc flush_dns_cache "${USER_ID}" >/dev/null 2>&1 || true
        _dig=$(ssh_lan "timeout 10 dig @${_gw} +time=3 +tries=1 +short ${_P18_DOMAIN} A" 2>/dev/null || true)
        ...
        if [[ "$_dig" == "${_P18_ANSWER}" ]] && \
           [[ "${_ttl_second_count:-0}" -eq $(( ${_ttl_first_count:-0} + 1 )) ]]; then
            pass "Step 73: DNS TTL expiry refetches upstream" \
                "upstream count ${_ttl_first_count}->${_ttl_second_count} after 7s"
        else
            fail "Step 73: DNS TTL expiry refetches upstream" \
    ...
    if ! _p18_add_secondary_dns_alias || ! _p18_start_secondary_responder; then
        fail "Step 74: DNS upstream failover" \
        ...
        if [[ "$_dig" == "${_P18_FAILOVER_ANSWER}" ]] && \
           [[ "$_failover_elapsed" -ge "${_P18_FAILOVER_TIMEOUT_SECS}" ]]; then
            pass "Step 74: DNS upstream failover" \
    ...
    if [[ "$_static" == "${WAN_IP}" ]] && \
       ! ssh_wan "pgrep -f '[f]astrg_dns_responder.py' >/dev/null 2>&1" 2>/dev/null; then
        pass "Step 75: DNS phase cleanup + static recovery" \
            "static answer=${WAN_IP}; responder absent; BRAS/PPPoE restored"
    else
        fail "Step 75: DNS phase cleanup + static recovery" \
