...
# ---------------------------------------------------------------------------
# Phase 15 — Prometheus /metrics route (Steps 62-66)
#
...
phase15_metrics_route() {
    bold "═══════════════════════════════════════════════════════"
    bold " Phase 15 — Prometheus /metrics route (Steps 62-66)"
    bold "═══════════════════════════════════════════════════════"
    ...
    _mport="${_mport_raw##*:}"
    if [[ -z "$_mport" ]]; then
        fail "Step 62: read MetricsListenPort" "fixture precondition missing: MetricsListenPort not set in /etc/fastrg/config.cfg"
        fail "Step 63: /metrics returns 200"     "fixture precondition missing: no metrics port"
        fail "Step 64: expected metric families" "fixture precondition missing: no metrics port"
        fail "Step 65: well-formed exposition"   "fixture precondition missing: no metrics port"
        fail "Step 66: unknown path returns 404"  "fixture precondition missing: no metrics port"
        return
    fi
    info "Metrics endpoint: ${FASTRG_NODE}:${_mport}/metrics"
    pass "Step 62: read MetricsListenPort" "port=${_mport}"
    ...
    if [[ "$_code" != "200" ]]; then
        fail "Step 63: /metrics returns 200" "got HTTP '${_code}'"
        skip "Step 64: expected metric families" "/metrics not reachable"
        skip "Step 65: well-formed exposition"   "/metrics not reachable"
        skip "Step 66: unknown path returns 404"  "/metrics not reachable"
        return
    fi
    local _lines
    _lines=$(printf '%s\n' "$_body" | wc -l | xargs)
    pass "Step 63: /metrics returns 200" "HTTP 200, ${_lines} lines"
    ...
    if [[ -z "$_missing" ]]; then
        ...
        fail "Step 64: expected metric families" "missing:${_missing}"
    fi
    ...
    if [[ -z "$_dups" ]]; then
        pass "Step 65: well-formed exposition" "no duplicate TYPE lines"
    else
        fail "Step 65: well-formed exposition" "duplicate TYPE: $(printf '%s' "$_dups" | tr '\n' ';')"
    fi
    ...
    if [[ "$_code404" == "404" ]]; then
        pass "Step 66: unknown path returns 404" "HTTP 404"
    else
        fail "Step 66: unknown path returns 404" "got HTTP '${_code404}'"
    fi
}
