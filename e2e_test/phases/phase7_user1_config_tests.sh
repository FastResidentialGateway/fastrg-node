...
phase7_user1_config_tests() {
    bold "═══════════════════════════════════════════════════════"
    bold " Phase 7 — User 1 gRPC→etcd Config Tests (Steps 15-22)"
    bold "═══════════════════════════════════════════════════════"
    ...
    # ------------------------------------------------------------------
    # Step 15 — ApplyConfig user 1 → etcd key written correctly
    # ------------------------------------------------------------------
    info "Step 15: ApplyConfig user ${U1} (VLAN=${U1_VLAN} account=${U1_ACCOUNT} pool=${U1_POOL_START}-${U1_POOL_END})..."
    ...
    if [[ -z "$_apply_status" ]]; then
        fail "Step 15: ApplyConfig user ${U1}" \
        ...
        skip "Step 18: DisconnectHsi user 1"          "ApplyConfig failed"
    ...
    if [[ -z "$_u1_etcd" ]]; then
        ...
        if [[ -z "$MISMATCH" ]]; then
            pass "Step 15: ApplyConfig user ${U1}" \
                "account=${_e_account} vlan=${_e_vlan} pool=${_e_pool} enableStatus=${_e_enable}"
        else
            fail "Step 15: ApplyConfig user ${U1}" "etcd mismatch:${MISMATCH}"
        fi
    fi

    # ------------------------------------------------------------------
    # Step 16 — fastrg watches etcd → applies user 1 config locally
    # ------------------------------------------------------------------
    info "Step 16: Verifying fastrg applied user ${U1} config locally (GetFastrgHsiInfo + GetFastrgDhcpInfo)..."
    ...
    if [[ -z "$_u1_hsi_user" ]] || [[ -z "$_u1_dhcp_user" ]]; then
        fail "Step 16: fastrg applies user ${U1} config" \
        ...
        if [[ -z "$MISMATCH" ]]; then
            pass "Step 16: fastrg applies user ${U1} config" \
                "vlan=${_g_vlan} pool=${_g_range} gw=${_g_gw}"
        else
            fail "Step 16: fastrg applies user ${U1} config" "Mismatch:${MISMATCH}"
        fi
    fi

    # ------------------------------------------------------------------
    # Step 17 — ConnectHsi user 1 → PPPoE phase starts
    # PPPoE will fail (no server for user 1) — that is expected.
    # We verify the command was processed by observing the phase change.
    # ------------------------------------------------------------------
    info "Step 17: ConnectHsi user ${U1} → verify PPPoE phase changes from 'End phase'..."
    ...
    if [[ $_disc_ok -eq 1 ]]; then
        pass "Step 18: DisconnectHsi user ${U1}" "PPPoE returned to 'End phase'"
    else
        fail "Step 18: DisconnectHsi user ${U1}" \
            "PPPoE still in '${_disc_phase:-unknown}' after DisconnectHsi + 15 s"
    fi

    # ------------------------------------------------------------------
    # Step 19 — DhcpServerStart user 1 → DHCP server on
    # ------------------------------------------------------------------
    info "Step 19: DhcpServerStart user ${U1} → verify DHCP server starts..."
    ...
    if [[ "$_dhcp15" == "DHCP server is on" ]]; then
        pass "Step 19: DhcpServerStart user ${U1}" "status='${_dhcp15}'"
    else
        fail "Step 19: DhcpServerStart user ${U1}" \
            "expected 'DHCP server is on', got '${_dhcp15:-empty}'"
    fi

    # ------------------------------------------------------------------
    # Step 20 — DhcpServerStop user 1 → DHCP server off
    # ------------------------------------------------------------------
    info "Step 20: DhcpServerStop user ${U1} → verify DHCP server stops..."
    ...
    if [[ "$_dhcp16" != "DHCP server is on" ]]; then
        pass "Step 20: DhcpServerStop user ${U1}" "status='${_dhcp16:-off}'"
    else
        fail "Step 20: DhcpServerStop user ${U1}" \
            "expected DHCP off, got '${_dhcp16}'"
    fi

    # ------------------------------------------------------------------
    # Step 21 — AddDnsRecord user 1 → etcd write + fastrg loads locally
    # ------------------------------------------------------------------
    info "Step 21: AddDnsRecord user ${U1} domain=${U1_DNS_DOMAIN} ip=${U1_DNS_IP} ttl=${U1_DNS_TTL}..."
    ...
    if [[ -z "$_dns_add_status" ]]; then
        fail "Step 21: AddDnsRecord user ${U1}" \
            "gRPC AddDnsRecord returned no status — response: $(printf '%s' "$_dns_add_reply")"
        skip "Step 22: RemoveDnsRecord user 1" "AddDnsRecord failed"
    else
        sleep 1
        ...
        if [[ -z "$MISMATCH" ]]; then
            pass "Step 21: AddDnsRecord user ${U1}" \
                "etcd ip=${_e17_ip} ttl=${_e17_ttl} grpc=ok"
        else
            fail "Step 21: AddDnsRecord user ${U1}" "Mismatch:${MISMATCH}"
        fi

        # ------------------------------------------------------------------
        # Step 22 — RemoveDnsRecord user 1 → etcd deleted + fastrg removes
        # ------------------------------------------------------------------
        info "Step 22: RemoveDnsRecord user ${U1} domain=${U1_DNS_DOMAIN}..."
        ...
        if [[ -z "$_dns_del_status" ]]; then
            fail "Step 22: RemoveDnsRecord user ${U1}" \
            ...
            if [[ -z "$MISMATCH" ]]; then
                pass "Step 22: RemoveDnsRecord user ${U1}" \
                    "etcd key deleted, fastrg record removed"
            else
                fail "Step 22: RemoveDnsRecord user ${U1}" "Mismatch:${MISMATCH}"
            fi
        fi
    fi
