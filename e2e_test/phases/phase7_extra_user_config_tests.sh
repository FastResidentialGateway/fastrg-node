...
phase7_extra_user_config_tests() {
    ...
    # ------------------------------------------------------------------
    # Step 19 — ApplyConfig new subscriber → etcd key written correctly
    # ------------------------------------------------------------------
    info "Step 19: ApplyConfig user ${U1} (VLAN=${U1_VLAN} account=${U1_ACCOUNT} pool=${U1_POOL_START}-${U1_POOL_END})..."
    ...
    # ------------------------------------------------------------------
    # Step 20 — fastrg watches etcd → applies user 1 config locally
    # ------------------------------------------------------------------
    info "Step 20: Verifying fastrg applied user ${U1} config locally (GetFastrgHsiInfo + GetFastrgDhcpInfo)..."
    ...
    if [[ -z "$_u1_hsi_user" ]] || [[ -z "$_u1_dhcp_user" ]]; then
        fail "Step 20: fastrg applies user ${U1} config" \
        ...
        if [[ -z "$MISMATCH" ]]; then
            pass "Step 20: fastrg applies user ${U1} config" \
                "vlan=${_g_vlan} pool=${_g_range} gw=${_g_gw}"
        else
            fail "Step 20: fastrg applies user ${U1} config" "Mismatch:${MISMATCH}"
        fi
    fi

    # ------------------------------------------------------------------
    # Step 21 — ConnectHsi user 1 → PPPoE phase starts
    # PPPoE will fail (no server for user 1) — that is expected.
    # We verify the command was processed by observing the phase change.
    # ------------------------------------------------------------------
    info "Step 21: ConnectHsi user ${U1} → verify PPPoE phase changes from 'End phase'..."
    ...
    if [[ $_disc_ok -eq 1 ]]; then
        pass "Step 22: DisconnectHsi user ${U1}" "PPPoE returned to 'End phase'"
    else
        fail "Step 22: DisconnectHsi user ${U1}" \
            "PPPoE still in '${_disc_phase:-unknown}' after DisconnectHsi + 15 s"
    fi

    # ------------------------------------------------------------------
    # Step 23 — DhcpServerStart user 1 → DHCP server on
    # ------------------------------------------------------------------
    info "Step 23: DhcpServerStart user ${U1} → verify DHCP server starts..."
    ...
    # ------------------------------------------------------------------
    # Step 24 — DhcpServerStop user 1 → DHCP server off
    # ------------------------------------------------------------------
    info "Step 24: DhcpServerStop user ${U1} → verify DHCP server stops..."
    ...
    # ------------------------------------------------------------------
    # Step 25 — AddDnsRecord user 1 → etcd write + fastrg loads locally
    # ------------------------------------------------------------------
    info "Step 25: AddDnsRecord user ${U1} domain=${U1_DNS_DOMAIN} ip=${U1_DNS_IP} ttl=${U1_DNS_TTL}..."
    ...
    if [[ -z "$_dns_add_status" ]]; then
        ...
        if [[ -z "$MISMATCH" ]]; then
            pass "Step 25: AddDnsRecord user ${U1}" \
        ...
        # ------------------------------------------------------------------
        # Step 26 — RemoveDnsRecord user 1 → etcd array updated + fastrg removes
        # ------------------------------------------------------------------
        info "Step 26: RemoveDnsRecord user ${U1} domain=${U1_DNS_DOMAIN}..."
        ...
        if [[ -z "$_dns_del_status" ]]; then
            fail "Step 26: RemoveDnsRecord user ${U1}" \
            ...
            if [[ -z "$MISMATCH" ]]; then
                pass "Step 26: RemoveDnsRecord user ${U1}" \
