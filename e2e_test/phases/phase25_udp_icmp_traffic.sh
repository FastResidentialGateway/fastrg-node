...
# ---------------------------------------------------------------------------
# Phase 25 — Sustained UDP / ICMP traffic across NAT timeout (Steps 101-103)
# ---------------------------------------------------------------------------
...
phase25_udp_icmp_traffic() {
    bold "═══════════════════════════════════════════════════════"
    bold " Phase 25 — Sustained UDP / ICMP Traffic (Steps 101-103)"
    bold "═══════════════════════════════════════════════════════"
    ...
    info "Step 101: running 20s UDP LAN→WAN flow at 50 Mbps..."
    ...
    info "Step 102: running 20s reverse UDP WAN→LAN flow at 50 Mbps..."
    _alloc_base=$(_p25_fetch_metric "fastrg_node_per_user_nat_alloc_fail_total")
    if [[ $_server_ready -eq 1 ]]; then
        if _iperf_out=$(ssh_lan \
    ...
    if [[ $_iperf_rc -eq 0 && $_client_stopped -eq 1 && $_server_stopped -eq 1 ]] && \
       ...
       _p25_is_uint "$_alloc_base" && [[ "$_alloc_after" == "$_alloc_base" ]]; then
        pass "Step 102: sustained reverse UDP WAN→LAN" \
            "loss=${_lost}%, bytes=${_bytes}, alloc_fail delta=0; full 20s reverse flow completed"
    else
        fail "Step 102: sustained reverse UDP WAN→LAN" \
    ...
    info "Step 103: running approximately 15s of ICMP echo traffic..."
    _entries_base=$(_p25_fetch_metric "fastrg_node_per_user_nat_entries_used")
    ...
    if [[ "$_ping_loss" != "0" && "$_ping_loss" != "0.0" ]]; then
        info "Step 103: first ping reported ${_ping_loss:-unparseable}% loss; retrying once..."
