...
phase25_udp_icmp_traffic() {
    ...
    # rows expose the per-queue distribution. The shared probe reflects 16 LAN
    # flows with distinct source ports off a WAN-side UDP echo server; each
    # flow gets a distinct NAT external port, so the echoed (downstream)
    # packets are 16 distinct inner tuples that WAN RSS must spread over
    # queues 1..N. e2e_rss_spread_verdict names what the deltas show: a
    # collapse to a single queue or onto queue 0 — the silent RSS/DDP
    # degradation this step exists for — is not a pass.
    # ------------------------------------------------------------------
    info "Step 107b: probing RSS distribution with ${_E2E_RSS_FLOWS} UDP echo flows..."
    local _rss_stats="" _rss_verdict="" _rss_detail=""
    local _wd_hit="" _wd_count="" _wd_sum="" _wc_delta="" _flows_ok=""

    _rss_stats=$(e2e_rss_probe 4 "${WAN_IP}")
    ...
    if [[ "$_rss_verdict" == "pass" ]]; then
        pass "Step 107b: RSS queue distribution" "$_rss_detail"
    else
        fail "Step 107b: RSS queue distribution" "verdict=${_rss_verdict}; ${_rss_detail}"
    fi
