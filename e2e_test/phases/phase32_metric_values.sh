...
# ---------------------------------------------------------------------------
# Phase 32 — Prometheus metric value assertions (Steps 129-135)
#
...
# its Step 128 has just proven both subscribers are in Data phase, which is
...
phase32_metric_values() {
    bold "═══════════════════════════════════════════════════════"
    bold " Phase 32 — Prometheus Metric Values (Steps 129-135)"
    bold "═══════════════════════════════════════════════════════"
    ...
    if [[ -z "$_body" ]] || ! e2e_is_uint "$_uc" || [[ "$_uc" -lt 1 ]] || \
       ! e2e_is_uint "$_max" || [[ "${_max:-0}" -lt 1 ]]; then
        ...
        fail "Step 129: PPPoE phase tallies"        "$_issue"
        fail "Step 130: DHCP lease and server gauges" "$_issue"
        fail "Step 131: DPDK mempool accounting"    "$_issue"
        fail "Step 132: DPDK heap and hugepage"     "$_issue"
        fail "Step 133: lcore cycle counters"       "$_issue"
        fail "Step 134: NIC info and node start time" "$_issue"
        fail "Step 135: unknown-user and NIC error counters" "$_issue"
        return 0
    fi

    # ------------------------------------------------------------------
    # Step 129 — the eight PPPoE phase gauges partition the subscriber
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 129: PPPoE phase tallies" \
    ...
    # ------------------------------------------------------------------
    # Step 130 — per-subscriber lease gauges match the configured pool
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 130: DHCP lease and server gauges" \
    ...
    # ------------------------------------------------------------------
    # Step 131 — every mempool row is self-consistent, and the two CCB
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 131: DPDK mempool accounting" \
    ...
    # ------------------------------------------------------------------
    # Step 132 — heap accounting per NUMA socket plus the hugepage gauge.
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 132: DPDK heap and hugepage" \
    ...
    # ------------------------------------------------------------------
    # Step 133 — lcore cycle counters over two samples. They are cumulative,
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 133: lcore cycle counters" \
    ...
    # ------------------------------------------------------------------
    # Step 134 — NIC info rows are proper info metrics (value 1, all labels
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 134: NIC info and node start time" \
    ...
    # ------------------------------------------------------------------
    # Step 135 — the counters no e2e scenario can reliably provoke. They are
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 135: unknown-user and NIC error counters" \
