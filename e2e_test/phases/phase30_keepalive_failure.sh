...
# ---------------------------------------------------------------------------
# Phase 30 — PPPoE runtime disconnect and keepalive recovery (Steps 121-124)
# ---------------------------------------------------------------------------
...
phase30_keepalive_failure() {
    ...
    bold "═══════════════════════════════════════════════════════"
    bold " Phase 30 — PPPoE Runtime Recovery (Steps 121-124)"
    bold "═══════════════════════════════════════════════════════"
    ...
    if [[ -n "$_issue119" ]]; then
        fail "Step 121: recover after LCP Terminate-Request" "dirty precondition: ${_issue119}"
        fail "Step 122: recover after peer PADT" "dirty precondition: ${_issue119}"
        fail "Step 123: detect LCP keepalive failure" "dirty precondition: ${_issue119}"
        fail "Step 124: restore default BRAS and auto-recover" "dirty precondition: ${_issue119}"
        return 0
    fi
    ...
    if [[ $_step119_ok -eq 1 ]]; then
        pass "Step 121: recover after LCP Terminate-Request" \
        ...
        fail "Step 121: recover after LCP Terminate-Request" "$_issue119"
    fi
    ...
    if [[ $_step120_ok -eq 1 ]]; then
        pass "Step 122: recover after peer PADT" \
        ...
        fail "Step 122: recover after peer PADT" "$_issue120"
    fi
    ...
    if [[ $_step121_ok -eq 1 ]]; then
        pass "Step 123: detect LCP keepalive failure" \
            "users ${SUB_IDS[*]} logged 3 missed echo-requests and left Data phase in ${_event_seconds}s"
    else
        fail "Step 123: detect LCP keepalive failure" "$_issue121"
    fi
    ...
    if [[ $_step122_ok -eq 1 ]]; then
        pass "Step 124: restore default BRAS and auto-recover" \
            "default echo behavior restored; users ${SUB_IDS[*]} auto-recovered in ${_recovery_seconds}s without connect_hsi"
    else
        fail "Step 124: restore default BRAS and auto-recover" "$_issue122"
    fi
