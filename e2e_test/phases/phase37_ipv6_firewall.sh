...
# ---------------------------------------------------------------------------
# Phase 37 — IPv6 stateful firewall (Steps 156-170)
#
...
# Steps 159/160 can tell this apart from a firewall that really dropped
# everything.
...
phase37_ipv6_firewall() {
    ...
    bold "═══════════════════════════════════════════════════════"
    bold " Phase 37 — IPv6 Stateful Firewall (Steps 156-170)"
    bold "═══════════════════════════════════════════════════════"

    # ------------------------------------------------------------------
    # Step 156 — bring IPv6 up end to end again
    # ------------------------------------------------------------------
    info "Step 156: enabling IPv6, redialling and provisioning both hosts..."
    _issue=""
    _P37_IPV6_SET=1
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 156: IPv6 firewall fixture ready" \
            "delegated ${_P37_PD_PREFIX}, LAN host ${_P37_LAN6}, WAN host ${_P37_WAN6_HOST_ADDR}"
    else
        fail "Step 156: IPv6 firewall fixture ready" \
            "${_issue}; lan addrs='${_lan_addrs:-none}'"
    fi

    # ------------------------------------------------------------------
    # Step 157 — the path is alive, so later drops can be blamed on the firewall
    # ------------------------------------------------------------------
    info "Step 157: ping6 LAN→WAN to prove the path works at all..."
    _ok=0
    ...
    if [[ $_ok -eq 1 ]]; then
        pass "Step 157: LAN→WAN ping6 works" \
            "${_P37_WAN6_HOST_ADDR} reachable with 0% packet loss"
    else
        fail "Step 157: LAN→WAN ping6 works" \
            "${_P37_WAN6_HOST_ADDR} - ${_P37_PING_DETAIL:-no response}; route: $(ssh_lan "ip -6 route get ${_P37_WAN6_HOST_ADDR}" 2>/dev/null | tr '\n' ' ' | cut -c 1-200 || true)"
    fi

    # ------------------------------------------------------------------
    # Step 158 — a reply to something the LAN sent is allowed back in
    # ------------------------------------------------------------------
    info "Step 158: LAN-initiated UDP must get its reply back..."
...
echo \$! >'${_P37_WAN_SRV_PID}'" >/dev/null 2>&1 || true
    ...
    if [[ $_ok -eq 1 ]] && printf '%s' "$_out" | grep -q 'echo-ok'; then
        pass "Step 158: LAN-initiated UDP gets its reply" \
            "datagram from ${_P37_LAN6}:41600 echoed back through the firewall"
    else
        fail "Step 158: LAN-initiated UDP gets its reply" \
    ...
    # ------------------------------------------------------------------
    # Step 159 — an unsolicited inbound ping never reaches the LAN
    # ------------------------------------------------------------------
    info "Step 159: unsolicited inbound ICMPv6 echo must be dropped..."
    _issue=""
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 159: unsolicited inbound ping is dropped" \
            "WAN→${_P37_LAN6} 100% loss and no echo request on ${_P37_LAN_VLAN}"
    else
        fail "Step 159: unsolicited inbound ping is dropped" "$_issue"
    fi

    # ------------------------------------------------------------------
    # Step 160 — an unsolicited inbound TCP SYN never reaches the LAN
    # ------------------------------------------------------------------
    info "Step 160: unsolicited inbound TCP SYN must be dropped..."
    _issue=""
...
PY" 2>&1 || true)
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 160: unsolicited inbound TCP SYN is dropped" \
            "connect to [${_P37_LAN6}]:22 failed and no SYN on ${_P37_LAN_VLAN}"
    else
        fail "Step 160: unsolicited inbound TCP SYN is dropped" "$_issue"
    fi

    # ------------------------------------------------------------------
    # Step 161 — an idle TCP connection outlives the flat 10 second timeout
    # ------------------------------------------------------------------
    ...
    info "Step 161: an idle TCP connection must survive 15s of silence..."
    _issue=""
    ssh_wan "rm -f '${_P37_WAN_SRV_PID}' '${_P37_WAN_SRV_OUT}';
...
PY" 2>&1 || true)
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 161: idle TCP session survives" \
            "server data after 15s of silence still reached the LAN client"
    else
        fail "Step 161: idle TCP session survives" \
            "${_issue}; server: $(ssh_wan "tail -n 3 '${_P37_WAN_SRV_OUT}' 2>/dev/null" 2>/dev/null | tr '\n' ' ' | cut -c 1-200 || true)"
    fi

    # ------------------------------------------------------------------
    # Step 162 — forged packets aimed at a live TCP session are dropped
    # ------------------------------------------------------------------
    info "Step 162: injecting state-mismatch and out-of-window TCP from the WAN..."
    _issue=""
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 162: forged TCP against a live IPv6 session is dropped" \
            "state-mismatch SYN and out-of-window ACK on [${_flow6}]:${_sport} both dropped"
    else
        fail "Step 162: forged TCP against a live IPv6 session is dropped" "$_issue"
    fi

    # ------------------------------------------------------------------
    # Step 163 — tcp_conntrack_enabled governs IPv6 exactly as it does IPv4
    # ------------------------------------------------------------------
    info "Step 163: checking the tcp_conntrack switch on the IPv6 path..."
    _issue=""
    if [[ ! "$_sport" =~ ^[0-9]+$ ]]; then
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 163: the tcp_conntrack switch governs IPv6 too" \
            "SYN forwarded to ${_P37_LAN_VLAN} while off, dropped again once back on"
    else
        fail "Step 163: the tcp_conntrack switch governs IPv6 too" "$_issue"
    fi

    # ------------------------------------------------------------------
    # Step 164 — an ICMPv6 error is judged by the packet it quotes
    # ------------------------------------------------------------------
    ...
    info "Step 164: forged and genuine ICMPv6 Packet Too Big..."
    _issue=""
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 164: ICMPv6 errors are judged by what they quote" \
            "the one quoting the live session passed, the forged one was dropped"
    else
        fail "Step 164: ICMPv6 errors are judged by what they quote" "$_issue"
    fi
    ...
    # ------------------------------------------------------------------
    # Step 165 — an expired session cannot be reopened from the WAN side
    # ------------------------------------------------------------------
    info "Step 165: an expired IPv6 session must not come back from the WAN..."
    _issue=""
...
echo \$! >'${_P37_LISTENER_PID}'" >/dev/null 2>&1 || true
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 165: an expired session stays closed" \
            "the live tuple reached [${_listen6}]:${_P37_REVIVE_LISTEN}, the same tuple was dropped after 15s idle"
    else
        fail "Step 165: an expired session stays closed" \
            "${_issue}; positive=${_positive_ok} negative=${_negative_seen} listener_alive=${_listener_alive}"
    fi

    # ------------------------------------------------------------------
    # Step 166 — idle IPv6 throughput baseline
    # ------------------------------------------------------------------
    info "Step 166: measuring the idle IPv6 throughput baseline..."
    _issue=""
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 166: IPv6 throughput baseline" \
            "$(_p37_mbps "$_P37_BASE_BPS") Mbps over IPv6 with an idle session table"
    else
        fail "Step 166: IPv6 throughput baseline" "$_issue"
    fi

    # ------------------------------------------------------------------
    # Step 167 — fill the session table to about 90%
    # ------------------------------------------------------------------
    info "Step 167: filling the IPv6 session table with ~59000 UDP flows..."
    _issue=""
    _ok=0
    ssh_lan "rm -f '${_P37_SPRAY_PID}' '${_P37_SPRAY_OUT}';
...
echo \$! >'${_P37_SPRAY_PID}'" >/dev/null 2>&1 || true
    ...
    if [[ $_ok -eq 1 && -z "$_issue" ]]; then
        pass "Step 167: IPv6 session table filled to near capacity" \
        ...
        fail "Step 167: IPv6 session table filled to near capacity" \
            "${_issue:+${_issue}; }ipv6_firewall_entries_used='${_used:-missing}' target=${_P37_FILL_TARGET}; sprayer: ${_spray_log:-no output}"
    fi

    # ------------------------------------------------------------------
    # Step 168 — throughput and new sessions while the table stays full
    # ------------------------------------------------------------------
    ...
    info "Step 168: measuring IPv6 throughput while the table stays near capacity..."
    _issue=""
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 168: the firewall holds up near capacity" \
            "$(_p37_mbps "$_bps") Mbps of $(_p37_mbps "$_P37_BASE_BPS") Mbps baseline with ${_used} live sessions, new flows still open"
    else
        fail "Step 168: the firewall holds up near capacity" \
            "${_issue}; ipv6_firewall_entries_used='${_used:-missing}'"
    fi

    # ------------------------------------------------------------------
    # Step 169 — stop the sprayer and watch the GC drain the table
    # ------------------------------------------------------------------
    info "Step 169: stopping the sprayer and waiting for the table to drain..."
    _issue=""
    ...
    if [[ -z "$_issue" ]]; then
        pass "Step 169: the GC drains the session table" \
            "ipv6_firewall_entries_used down to ${_used} (< ${_P37_DRAIN_TARGET}), IPv6 still forwarding"
    else
        fail "Step 169: the GC drains the session table" "$_issue"
    fi

    # ------------------------------------------------------------------
    # Step 170 — put the bench back to the canonical IPv4 fixture
    # ------------------------------------------------------------------
    info "Step 170: restoring the bench and rechecking the canonical fixture..."
    _issue=""
