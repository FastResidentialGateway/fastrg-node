...
# ---------------------------------------------------------------------------
# Phase 20 — NAT idle expiry and amortized GC metrics (Steps 77-79)
# ---------------------------------------------------------------------------
...
phase20_nat_expiry() {
    bold "═══════════════════════════════════════════════════════"
    bold " Phase 20 — NAT idle expiry + GC metrics (Steps 77-79)"
    bold "═══════════════════════════════════════════════════════"
    ...
    info "Step 77: creating ${_flow_count} UDP mappings with distinct LAN source ports..."
...
PY"; then
    ...
    if [[ $_step77_ok -eq 1 && "$_alloc_cur" == "$_alloc_base" ]]; then
        ...
        pass "Step 77: NAT entry creation metrics" \
            "entries delta=$(( _entries_cur - _entries_base )) (target>=${_flow_count}); alloc_fail delta=0"
    else
        fail "Step 77: NAT entry creation metrics" \
    ...
    info "Step 78: starting sustained iperf3 traffic while idle UDP mappings expire..."
    ...
    info "Step 79: checking live UDP inbound control and expired inbound rejection..."
