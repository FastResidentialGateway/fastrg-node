...
# ---------------------------------------------------------------------------
# Phase 14 — DNS Reconnect Stress Test (Steps 52-61)
#
#   Step 52-61  10 rounds of disconnect → reconnect → DNS static verify
...
phase14_stress_test() {
    ...
    bold "═══════════════════════════════════════════════════════"
    bold " Phase 14 — DNS Reconnect Stress Test (Steps 52-61)"
    bold "═══════════════════════════════════════════════════════"
    ...
    _stress_check_dns() {
        ...
        if [[ -z "$DNS_DOMAINS" ]]; then
            fail "$_step_name" "fixture precondition missing: no DNS static records in etcd (canonical fixture guarantees them)"
            return 0
        fi
