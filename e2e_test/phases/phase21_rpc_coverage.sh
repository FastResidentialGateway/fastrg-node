...
# ---------------------------------------------------------------------------
# Phase 21 — Read-only RPC coverage (Steps 92-94)
# ---------------------------------------------------------------------------
...
phase21_rpc_coverage() {
    bold "═══════════════════════════════════════════════════════"
    bold " Phase 21 — Read-only RPC Coverage (Steps 92-94)"
    bold "═══════════════════════════════════════════════════════"
...
PY
    ...
    if [[ $_arp_positive_ok -eq 1 && $_arp_negative_ok -eq 1 ]]; then
        pass "Step 92: GetArpTable" "${_arp_detail}; invalid user rejected (rc=${_invalid_rc})"
    else
        _arp_snippet=$(_p21_response_snippet "$_arp")
        _invalid_snippet=$(_p21_response_snippet "$_invalid_out")
        fail "Step 92: GetArpTable" \
...
PY
        ...
        pass "Step 93: GetFastrgSystemXStats" "$_xstats_detail"
    else
        _xstats_snippet=$(_p21_response_snippet "$_xstats")
        fail "Step 93: GetFastrgSystemXStats" "response='${_xstats_snippet:-empty}'"
    fi
...
PY
        ...
        pass "Step 94: GetNodeStatus" "$_status_detail"
    else
        _status_snippet=$(_p21_response_snippet "$_status")
        fail "Step 94: GetNodeStatus" "response='${_status_snippet:-empty}'"
    fi
}
