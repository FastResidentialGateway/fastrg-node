...
phase16_rcu_concurrency() {
    ...
    if [[ "${_cnt_orig:-0}" -le 0 ]]; then
        skip "Step 67: writer churn under load" "cannot read subscriber count"
        skip "Step 68: data plane survived load+churn" "cannot read subscriber count"
        return
    fi
    ...
    if [[ -n "$cnt3" ]]; then
        pass "Step 68: data plane survived load+churn" \
            "node responsive after load+churn (num_users=${cnt3}, iperf3=${mbps} Mbps)"
    else
        fail "Step 68: data plane survived load+churn" "node unresponsive after load+churn"
    fi
