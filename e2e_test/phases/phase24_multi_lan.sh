...
# ---------------------------------------------------------------------------
# Phase 24 — Multiple virtual LAN devices (Steps 102-104)
# ---------------------------------------------------------------------------
...
phase24_multi_lan() {
    bold "═══════════════════════════════════════════════════════"
    bold " Phase 24 — Multiple LAN Devices (Steps 102-104)"
    bold "═══════════════════════════════════════════════════════"
...
PY
    ...
    if [[ $_step99_ok -eq 1 ]]; then
        pass "Step 103: MAC table per-device mappings" "$_arp_detail"
    else
        fail "Step 103: MAC table per-device mappings" \
            "Step 102 ready=${_step98_ok}; response='$(_p24_snippet "$_arp")'; expected=${_P24_LEASE_IPS[0]:-NA}/${_P24_MACS[0]},${_P24_LEASE_IPS[1]:-NA}/${_P24_MACS[1]},${_P24_LEASE_IPS[2]:-NA}/${_P24_MACS[2]}"
    fi
    ...
    if [[ $_step98_ok -eq 1 ]]; then
        ...
        # cache state after the synthetic ARP traffic in Step 103.
...
echo \$! >'${_P24_WAN_LISTENER_PID}'" >/dev/null 2>&1 || _step100_ok=0
    ...
    if [[ $_step100_ok -eq 1 ]]; then
        pass "Step 104: NAT mappings for multiple source IPs" \
        ...
        fail "Step 104: NAT mappings for multiple source IPs" \
            "Step 102 ready=${_step98_ok}; gateway_mac='${_gateway_mac}'; listener=${_listener_ready} output='$(_p24_snippet "$_listener_out")'; send=${_send_ok}; entries=${_nat_base:-NA}->${_nat_peak:-NA} delta=${_nat_delta}; WAN ports=${_received_ports}; lease=${_lease_after:-NA}/${_expected_count}; sends=[${_send_marks}]; series=[${_nat_series% }]"
    fi
